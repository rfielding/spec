...
`examples/spec_model.convspec` is a first model of this project in its own language. It is intentionally similar in spirit to Swagger: protobuf defines the serialized messages, and convspec defines the observable protocol, probabilities, actor capacity, assertions, and documentation views around those messages. The root spec owns actors/imports and includes `examples/spec_model_compile_measure_render.convspec` for the conversation.
...
- every actor has one spec-wide capacity for unread messages, and writes block when that capacity is full
...
The compiler does not yet derive queue arrival rates from upstream message writes or run the full MDP. The spec is written so those pieces have obvious homes: message traffic produces byte events, chance nodes produce transition probabilities, actor capacity provides queue bounds for performance metrics, and `(metric ...)` declarations identify the line and pie charts to compute from causal traffic.
